...
•  Grammar tells us that a statistical graphic, drawn on a coordinate system, is a mapping from data to aesthetic attributes (color, shape, size) of geometric objects (points, lines, bars) potentially containing statistical transformations and facets of the data.  (by facet he means something like color of diamonds graphed against price)
...
•  geoms  
•  stats  
•  scales (can be a attribute or a color) 
•  coord system   
...
2.  One or more layers   
    ...
    •	Position adjustment  
    •	Dataset  
    •	Aesthetic mappings (optional)(choice of axis for the variable)  
3.  Scale  
4.  Coordinate system  
...
#What is a Facet  (color of the diamond is the facet, create multiple graphs that map weight to x and price to y, clarity to x and price to y, etc)
...
data(diamonds)
str(diamonds)
str(diamonds[1]) #[ returns a dataframe
str(diamonds$carat)
str(diamonds[[1]]) #[[ returns a number
str(diamonds$cut) #cut is an ordinal factor with 5 levels
str(diamonds[2])
str(diamonds[[2]])
class(diamonds[[2]])
#note that if you click on the diamonds in the top right window it gives you a nice table summary
```

Let's create a simple bat graph of cut several ways
```{r}
cut_bargraph_1 <- ggplot(diamonds, aes(x=diamonds$cut)) # maps the x axis to diamonds$cut. obviously the diamonds refers to the dataframe, this uses names not indices
str(cut_bargraph_1) #shows you the structure of your plot
cut_bargraph_1 + geom_bar() # this adds the bar graph to this ggplot object
#cut_bargraph_1 + geom_line() # this doesn't work
str(cut_bargraph_1)

cut_bargraph_2 <- ggplot(diamonds[2], aes(x=diamonds[[2]])) #create a new plot using index, diamond[2] is a data frame, then we assign the vector diamonds[[2]] to the x axis
str(cut_bargraph_2)
cut_bargraph_2 <- cut_bargraph_2 + geom_bar(fill="gray", colour="red") + labs(x=names(diamonds)[[2]]) # he does this instead of adding like he did in version1.  notice the labs function that allows him to get at the names of the attributes and he relabeled the x axis to the name of the vector.  he used the name function to bring back the list of names in the dataframe diamonds.  the [[2]] returns just the name that is in the second position of the new vector
str(cut_bargraph_2) # see the labels have changed and maybe someother things have changed
print(cut_bargraph_2)  #renders our graph
str(cut_bargraph_2)
str(cut_bargraph_2[2])
cut_bargraph_2[2]  # 
str(names(diamonds))
str(names(diamonds)[2]) # same as the line below
str(names(diamonds)[[2]])
colnames(diamonds)[2] # another way of getting at the names
rownames(diamonds) # returns a shit ton of stuff 
# you can assign a list of names to your names < - values
...
```{r}
Test_Frame <- diamonds[1] #creates a dataframe called test frame and assign it the first column of diamonds (it will also be a data frame)
bin50 <- (max(Test_Frame[[1]])-min(Test_Frame[[1]]))/50  #this is how we get 50 bins in a plot.  you take the max value in the test frame and the min value and you divide by 50, this will give you 50 equally spaced bins.  a histogram is a nonparametric distribution of the data.  sometimes you have bimodal distributions.
Test_Plot <- ggplot(Test_Frame, aes(x=Test_Frame[[1]])) #create a test plot, you send it your dataframe and the first element in the dataframe as a label
...
```{r}
Test_Plot <- Test_Plot + aes(y=..density..) #the .. is special to ggplot

Test_Plot + labs(y="Density") #density is good when you don't have equal scale
```
...
```{r}
Test_Frame <- diamonds
binsizes <- c(3, 10, 20, 50, 75, 100) #here we look at different bin sizes using a for loop, you want to pick the one that isn't overkill but also doesn't hide issues with the data
...
Handle_Num <- function(Num_Frame) 
 {#same thing as above but using num_plot
...
Handle_Num2 <- function(Num_Frame) 
{
  bin50 <- (max(Num_Frame[[1]])-min(Num_Frame[[1]]))/50
  Num_Plot <- ggplot(Num_Frame, aes(x=Num_Frame[[1]]),
                    environment = environment())# envrionment = environment() makes our environment global
  Num_Plot <- Num_Plot + aes(y=..density..) +
...
Handle_Num2(diamonds[1]) #there is a bug in ggplot.  Num_frame does exist.
#ggplot assumes a global environment.  the dataframe that we are referencing is in a global environment but we are not.  we are in a local 
...
Plot_Num <- function(Num_Frame, Bin_Sizes) 
{
  for (i in 1:length(Num_Frame))
  {
    for (bins in 1:length(Bin_Sizes))
    {
      ...
      print(Num_Plot) # adds the label that we created above
